...
export function getBuildingBlockHeights() {
  ...
  for (let i = 0; i < BUILDINGS_PER_BLOCK; i++) {
    buildingHeights.push(
      Math.floor(Math.random() * (MAX_BUILDING_STORIES - 1)) * STORY_HEIGHT +
        STORY_HEIGHT,
    );
  }
...
export function getBlock(
  ...
  const sidewalk = getSidewalk(
    gl,
    glMatrix,
    textureMap.getTexture("sidewalk"),
    textureMap.getTexture("sidewalkNormal"),
  );
