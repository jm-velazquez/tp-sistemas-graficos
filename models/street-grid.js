...
import { BLOCK_SIDE, BLOCK_CORNER_RADIUS } from "./sidewalk.js";
...
function getLanesBuffers() {
	...
	for (let i = 0; i < 4; i++) {
		for (let j = 0; j < 5; j++) {
			...
			buffers.positionBuffer.push(...horizontalStreetBuffers.positionBuffer);
...
export function getStreetGrid(gl, textureMap) {
	...
	const intersectionsBuffers = getIntersectionsBuffers(lanesBuffers.indexBufferOffset);
	buffers.positionBuffer.push(...lanesBuffers.positionBuffer, ... intersectionsBuffers.positionBuffer);
	buffers.normalBuffer.push(...lanesBuffers.normalBuffer, ... intersectionsBuffers.normalBuffer);
	buffers.uvBuffer.push(...lanesBuffers.uvBuffer, ... intersectionsBuffers.uvBuffer);
	buffers.indexBuffer.push(...lanesBuffers.indexBuffer, ... intersectionsBuffers.indexBuffer);
