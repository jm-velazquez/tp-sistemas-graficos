export function getStreetBuffers(streetWidth, streetLength, xOffset = 0, yOffset = 0, isHorizontal = false,
																lengthUVMultiplier = 1, indexBufferOffset = 0) {
	...
	const positionBuffer = [
		xOffset - xSide / 2, yOffset - ySide / 2, 0,
		xOffset - xSide / 2, yOffset + ySide / 2, 0,
		xOffset + xSide / 2, yOffset - ySide / 2, 0,
		xOffset - xSide / 2, yOffset + ySide / 2, 0,
		xOffset + xSide / 2, yOffset - ySide / 2, 0,
	...
	if (isHorizontal) {
		uvBuffer = [
			0.5, 0,
			0  , 0,
			0.5, 1 * lengthUVMultiplier,
			0  , 0,
			0.5, 1 * lengthUVMultiplier,
			0  , 1 * lengthUVMultiplier,
		];
	} else {
		uvBuffer = [
			0  , 0,
