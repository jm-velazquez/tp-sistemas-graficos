...
export class Model {
    ...
    getWorldCoordinates(parentMatrix) {
        ...
        const worldCoordinates = vec4.fromValues(0, 0, 0, 1)
        vec4.transformMat4(worldCoordinates, worldCoordinates, matrix)

        const childrenWorldCoordinates = this.children.map((child) =>
            child.getWorldCoordinates(matrix)
        )
        childrenWorldCoordinates.unshift(worldCoordinates)
        return childrenWorldCoordinates
    }

    draw(gl, glProgram, parentMatrix, viewMatrix, projMatrix, lightParameters) {
        this.updateModelMatrix()
        let matrix = mat4.create()
        mat4.multiply(matrix, parentMatrix, this.modelMatrix)
        ...
        if (
            ...
            if (this.normalMap) {
                ...
                gl.uniform1i(
                    gl.getUniformLocation(glProgram, 'uSamplerNormal'),
                    1
                )
            }
